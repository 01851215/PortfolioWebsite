...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function createProgram(vs: any, fs: any) {
      let p = gl.createProgram(); gl.attachShader(p, vs); gl.attachShader(p, fs); gl.linkProgram(p);
    ...
    function getUniforms(program: any) {
      ...
      for (let i = 0; i < count; i++) { let n = gl.getActiveUniform(program, i).name; u[n] = gl.getUniformLocation(program, n); }
      return u;
    }
    ...
    const displayShaderSource = `
      precision highp float; precision highp sampler2D; varying vec2 vUv; varying vec2 vL; varying vec2 vR; varying vec2 vT; varying vec2 vB; uniform sampler2D uTexture; uniform sampler2D uDithering; uniform vec2 ditherScale; uniform vec2 texelSize;
    ...
    function createDoubleFBO(w: number, h: number, iF: any, f: any, t: any, p: any) {
      let fbo1 = createFBO(w, h, iF, f, t, p); let fbo2 = createFBO(w, h, iF, f, t, p);
    ...
    function resizeFBO(target: any, w: number, h: number, iF: any, f: any, t: any, p: any) {
      let n = createFBO(w, h, iF, f, t, p); copyProgram.bind(); gl.uniform1i(copyProgram.uniforms.uTexture, target.attach(0)); blit(n); return n;
    }
    ...
    function updateKeywords() { let k: string[] = []; if (config.SHADING) k.push('SHADING'); displayMaterial.setKeywords(k); }
    ...
    function resizeCanvas() {
      let w = scaleByPixelRatio(canvas!.clientWidth); let h = scaleByPixelRatio(canvas!.clientHeight);
    ...
    function step(dt: number) {
      ...
      let vid = velocity.read.attach(0); gl.uniform1i(advectionProgram.uniforms.uVelocity, vid); gl.uniform1i(advectionProgram.uniforms.uSource, vid); gl.uniform1f(advectionProgram.uniforms.dt, dt); gl.uniform1f(advectionProgram.uniforms.dissipation, config.VELOCITY_DISSIPATION); blit(velocity.write); velocity.swap();
    ...
    function drawDisplay(target: any) {
      let w = !target ? gl.drawingBufferWidth : target.width; let h = !target ? gl.drawingBufferHeight : target.height;
      displayMaterial.bind(); if (config.SHADING) gl.uniform2f(displayMaterial.uniforms.texelSize, 1.0 / w, 1.0 / h);
      gl.uniform1i(displayMaterial.uniforms.uTexture, dye.read.attach(0)); blit(target);
    }

    function splatPointer(pointer: any) { let dx = pointer.deltaX * config.SPLAT_FORCE; let dy = pointer.deltaY * config.SPLAT_FORCE; splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color); }

    function clickSplat(pointer: any) { const color = generateColor(); color.r *= 10.0; color.g *= 10.0; color.b *= 10.0; let dx = 10 * (Math.random() - 0.5); let dy = 30 * (Math.random() - 0.5); splat(pointer.texcoordX, pointer.texcoordY, dx, dy, color); }
    ...
    function correctDeltaX(d: number) { let ar = canvas!.width / canvas!.height; if (ar < 1) d *= ar; return d; }
    function correctDeltaY(d: number) { let ar = canvas!.width / canvas!.height; if (ar > 1) d /= ar; return d; }

    function generateColor() { let c = HSVtoRGB(Math.random(), 1.0, 1.0); c.r *= 0.15; c.g *= 0.15; c.b *= 0.15; return c; }

    function HSVtoRGB(h: number, s: number, v: number) {
      let r = 0, g = 0, b = 0; let i = Math.floor(h * 6); let f = h * 6 - i; let p = v * (1 - s); let q = v * (1 - f * s); let t = v * (1 - (1 - f) * s);
      switch (i % 6) { case 0: r=v;g=t;b=p; break; case 1: r=q;g=v;b=p; break; case 2: r=p;g=v;b=t; break; case 3: r=p;g=q;b=v; break; case 4: r=t;g=p;b=v; break; case 5: r=v;g=p;b=q; break; }
      return { r, g, b };
    }

    function wrap(v: number, min: number, max: number) { const r = max - min; if (r === 0) return min; return ((v - min) % r) + min; }
    function getResolution(resolution: number) { let ar = gl.drawingBufferWidth / gl.drawingBufferHeight; if (ar < 1) ar = 1.0 / ar; const min = Math.round(resolution); const max = Math.round(resolution * ar); if (gl.drawingBufferWidth > gl.drawingBufferHeight) return { width: max, height: min }; else return { width: min, height: max }; }
    function scaleByPixelRatio(input: number) { return Math.floor(input * (window.devicePixelRatio || 1)); }
    function hashCode(s: string) { if (s.length === 0) return 0; let h = 0; for (let i = 0; i < s.length; i++) { h = (h << 5) - h + s.charCodeAt(i); h |= 0; } return h; }
    ...
    let firstMove = false;
    function handleMouseMove(e: MouseEvent) { let p = pointers[0]; let px = scaleByPixelRatio(e.clientX); let py = scaleByPixelRatio(e.clientY); if (!firstMove) { updatePointerMoveData(p, px, py, generateColor()); firstMove = true; } else { updatePointerMoveData(p, px, py, p.color); } }
    function handleTouchStart(e: TouchEvent) { let ts = e.targetTouches; let p = pointers[0]; for (let i = 0; i < ts.length; i++) updatePointerDownData(p, ts[i].identifier, scaleByPixelRatio(ts[i].clientX), scaleByPixelRatio(ts[i].clientY)); }
    function handleTouchMove(e: TouchEvent) { let ts = e.targetTouches; let p = pointers[0]; for (let i = 0; i < ts.length; i++) updatePointerMoveData(p, scaleByPixelRatio(ts[i].clientX), scaleByPixelRatio(ts[i].clientY), p.color); }
    function handleTouchEnd(e: TouchEvent) { let ts = e.changedTouches; let p = pointers[0]; for (let i = 0; i < ts.length; i++) updatePointerUpData(p); }
